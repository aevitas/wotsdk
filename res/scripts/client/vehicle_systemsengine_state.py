...
import BigWorld
from VehicleEffects import clamp
...
class DetailedEngineStateWWISE(DetailedEngineState):
    ...
    def refresh(self, delta):
        ...
        if not self._vehicle.isPlayerVehicle or self._vehicle.physicsMode == VEHICLE_PHYSICS_MODE.STANDARD:
            ...
            self._gearNum = math.ceil(math.floor(math.fabs(speed) * 50) / 50 / speedRangeGear)
            ...
            self._rpm = 0
            if self._gearNum != 0:
                self._rpm = math.fabs(1 + (speed - self._gearNum * speedRangeGear) / speedRangeGear)
                self._rpm = self._rpm * (1.0 - self._GEAR_DELTA * self._gearNum) + self._GEAR_DELTA * self._gearNum
                self._reativelRPM = self._rpm
                self._rpm *= 100.0
            else:
                self._reativelRPM = 0.0
        else:
