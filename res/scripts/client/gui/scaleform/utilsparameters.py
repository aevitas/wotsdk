...
def getCurrentVehicleParams():
    ...
    if g_currentVehicle.isPresent() and GUI_SETTINGS.technicalInfo:
...
def getTurretParams(item, vehicle, callback):
    ...
    if GUI_SETTINGS.technicalInfo:
        params['parameters'] = (('armor', '%d/%d/%d' % item.descriptor['primaryArmor']),
         ...
         ('circularVisionRadius', BigWorld.wg_getIntegralFormat(item.descriptor['circularVisionRadius'])),
...
def getOptionalDeviceParams(item, vehicle, callback):
    ...
    parameters = {'parameters': [],
     'compatible': ()}
    if weight and weight != '0' and GUI_SETTINGS.technicalInfo:
